...
async function main() {
  ...
  if (!directDatabaseUrl) {
    console.error('[reset-and-import-sql] DIRECT_DATABASE_URL is required. No database operation was performed.');
    process.exit(1);
  }
  ...
  try {
    ...
    for (const record of records) {
      try {
        ...
        const specMm = parseFloat(record.规格mm) || null;
        const insertResult = await client.query(
          `
            INSERT INTO raw_materials
            (code, name, category, supplier, spec_mm, shape, notes, remaining, unit_cost, unit, low_stock_alert, created_at, updated_at)
        ...
        failCount++;
        console.error(`✗ [失败 ${failCount}] ${record.原料编码}: 导入该记录失败`);
      }
    }
    ...
    console.log(`库存变动记录总数: ${totalMaterials.rows[0].count}`);
  } catch {
    console.error('\n导入失败。数据库操作已停止。');
  } finally {
    await client.end();
