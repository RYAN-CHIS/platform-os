...
export default async function PreviewSeries({
  params,
  searchParams,
}: {
  ...
  const content = await getPreviewContent("series", id);

  if (!content) {
    return (
      <div style={{ padding: 60, textAlign: "center", fontFamily: "system-ui, sans-serif" }}>
        <h1 style={{ fontSize: 24, color: "#78716c", marginBottom: 8 }}>系列不存在</h1>
        <p style={{ color: "#a8a29e", fontSize: 14 }}>该系列可能已被删除或 ID 无效。</p>
      </div>
    );
  }
